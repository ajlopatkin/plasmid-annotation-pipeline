...
def main(plasmid_name):
	...
	pls_rec=[]
	with open("./../output/plasmids/" + plasmid_name + "/final.tsv") as plstsv:
	...
	length=0
	pls_loc=[]
	with open("./../output/plasmids/" + plasmid_name + "/final.gff") as plsgff:
		...
		for i in range(0, len(pls_loc)):
			...
			if len(pls_rec[i][9])>2:
				...
				if "antibiotic" in description.lower() or "antimicrobial resistance" in description.lower():
					...
					print("REPLICATION")
					color="pink"
				elif "conjugation" in description.lower() or "replication" in description.lower() or "pilus" in description.lower() or "plasmid" in description.lower() or "dna binding" in description.lower():
					print("CONJUGATION")
					color="teal"
                                        #continue
