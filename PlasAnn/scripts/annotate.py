...
def main(plasmid):
    ...
    #change the header for the FASTA file
    initial_fasta = open("./../fastas/" + plasmid + ".fasta", 'r')
    new_fasta = open("./../fastas/tmp.fasta", 'w')
    first_row = True
    for row in initial_fasta:
        if first_row:
            row = '>' + plasmid + "\n"
            first_row = False
        new_fasta.write(row)
    
    os.remove("./../fastas/" + plasmid + ".fasta")
    os.rename("./../fastas/tmp.fasta", "./../fastas/" + plasmid + ".fasta")
